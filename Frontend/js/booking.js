...
const querySalonId = queryParams.get("id");
const salonId = querySalonId || bookingForm?.dataset.salonId;
...
async function loadSalonData() {
  if (!salonId) {
    bookingMessage.textContent = "Please open booking from a salon details page.";
    return;
  }
...
bookingForm?.addEventListener("submit", async (event) => {
  ...
  const response = await fetch(`/booking/${salonId}/create/`, {
    method: "POST",
    headers: {
  ...
  if (response.ok) {
    ...
    localStorage.setItem(
      "pendingBooking",
      JSON.stringify({
        salon_name: salonName?.textContent || "Selected Salon",
        service_name: selectedService?.name || "Not selected",
        employee_name: specialistName,
        booking_date: bookingDateInput.value || "Not selected",
        start_time: selectedTimeInput.value || "Not selected",
        service_price: selectedService?.price || "0",
        service_duration: selectedService ? `${selectedService.duration_minutes} minutes` : "Not selected",
      })
    );
    window.location.href = data.redirect_to || "/payment/";
  }
});
